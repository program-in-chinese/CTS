...
namespace ts {
    ...
    export function emitFiles(resolver: EmitResolver, host: EmitHost, targetSourceFile: SourceFile, emitOnlyDtsFiles?: boolean, transformers?: TransformerFactory<SourceFile>[]): EmitResult {
        ...
        function emitSourceFileOrBundle({ jsFilePath, sourceMapFilePath, declarationFilePath }: EmitFileNames, sourceFileOrBundle: SourceFile | Bundle) {
            ...
            if (compilerOptions.转译声明) {
                写声明文件(jsFilePath, getOriginalSourceFileOrBundle(sourceFileOrBundle), host, emitterDiagnostics);
            }
            if (!host.isEmitBlocked(jsFilePath) && !compilerOptions.noEmit) {
                if (!emitOnlyDtsFiles && !compilerOptions.转译声明) {
    ...
    export function createPrinter(printerOptions: PrinterOptions = {}, handlers: PrintHandlers = {}): Printer {
        ...
        function needsDotDotForPropertyAccess(expression: Expression) {
            ...
            if (isNumericLiteral(expression)) {
                ...
                return !expression.numericLiteralFlags
                    && text.indexOf(令牌转为关键字(SyntaxKind.DotToken)) < 0;
            }
        ...
        function writeTokenText(token: SyntaxKind, pos?: number) {
            const tokenString = 令牌转为关键字(token);
            write(tokenString);
        ...
        function 取节点发射文本(node: Identifier | LiteralExpression) {
            const 别名 = node.别名;
            if (别名) {
                if (别名.旗帜 & 别名旗帜.汉英) {
                    return 别名.名称;
        ...
        function getTextOfNode(node: Node, includeTrivia?: boolean): string {
            if (isGeneratedIdentifier(node)) {
                ...
                return unescapeLeadingUnderscores(取节点发射文本(node));
            }
                ...
                return 取节点发射文本(node) as string;
            }

            return getSourceTextOfNodeFromSourceFile(currentSourceFile, node, 使用场景.输出, includeTrivia);
        }
