...
namespace ts {
    ...
    export interface System {
        ...
        appendFile?(path: string, data: string, writeByteOrderMark?: boolean): void;
        /**
    ...
    export let sys: System = (function () {
        function getNodeSystem(): System {
            ...
            function appendFile(fileName: string, data: string, writeByteOrderMark?: boolean): void {
                if (writeByteOrderMark) {
                    data = "\uFEFF" + data;
                }
                ...
                try {
                    fd = _fs.openSync(fileName, "a+");
                    _fs.appendFileSync(fd, data, /*position*/ undefined, "utf8");
                }
                finally {
                    if (fd !== undefined) {
                        _fs.closeSync(fd);
        ...
        function getChakraSystem(): System {
            ...
            return {
                ...
                appendFile(path: string, data: string, writeByteOrderMark?: boolean) {
                    // If a BOM is required, emit one
                    if (writeByteOrderMark) {
                        data = "\uFEFF" + data;
                    }

                    ChakraHost.appendFile(path, data);
                },
