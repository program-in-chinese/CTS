...
namespace ts {
    ...
    function emitDeclarations(host: EmitHost, resolver: EmitResolver, emitterDiagnostics: DiagnosticCollection, declarationFilePath: string,
        ...
        let writeTextOfNode: (text: string, node: Node, 文件种类: 文件种类) => void;
                ...
                if (errorInfo) {
                    if (errorInfo.typeName) {
                        emitterDiagnostics.add(createDiagnosticForNode(symbolAccessibilityResult.errorNode || errorInfo.errorNode,
                            ...
                            getTextOfNodeFromSourceText(currentText, errorInfo.typeName, 文件种类),
        ...
        function emitType(type: TypeNode | Identifier | QualifiedName) {
            switch (type.kind) {
                ...
                case SyntaxKind.LiteralType:
                    return writeTextOfNode(currentText, type, 文件种类);
            ...
            function writeEntityName(entityName: EntityName | Expression) {
                if (entityName.kind === SyntaxKind.Identifier) {
                    writeTextOfNode(currentText, entityName, 文件种类);
                }
                else {
                    ...
                    write(".");
                    writeTextOfNode(currentText, right, 文件种类);
                }
            }
            ...
            function emitTypePredicate(type: TypePredicateNode) {
                writeTextOfNode(currentText, type.parameterName, 文件种类);
                write(" is ");
                emitType(type.type);
            }
        ...
        function emitExportAssignment(node: ExportAssignment) {
            if (node.expression.kind === SyntaxKind.Identifier) {
                ...
                writeTextOfNode(currentText, node.expression, 文件种类);
            }
            else {
        ...
        function writeImportEqualsDeclaration(node: ImportEqualsDeclaration) {
            ...
            write("import ");
            writeTextOfNode(currentText, node.name, 文件种类);
            write(" = ");
        ...
        function writeImportDeclaration(node: ImportDeclaration) {
            ...
            if (node.importClause) {
                ...
                if (node.importClause.name && resolver.isDeclarationVisible(node.importClause)) {
                    writeTextOfNode(currentText, node.importClause.name, 文件种类);
                }
                if (node.importClause.namedBindings && isVisibleNamedBinding(node.importClause.namedBindings)) {
                    ...
                    if (node.importClause.namedBindings.kind === SyntaxKind.NamespaceImport) {
                        write("* as ");
                        writeTextOfNode(currentText, (<NamespaceImport>node.importClause.namedBindings).name, 文件种类);
                    }
                    else {
                        write("{ ");
        ...
        function emitExternalModuleSpecifier(parent: ImportEqualsDeclaration | ImportDeclaration | ExportDeclaration | ModuleDeclaration) {
            ...
            writeTextOfNode(currentText, moduleSpecifier, 文件种类);
        }

        function emitImportOrExportSpecifier(node: ImportOrExportSpecifier) {
            if (node.propertyName) {
                writeTextOfNode(currentText, node.propertyName, 文件种类);
                write(" as ");
            }
            writeTextOfNode(currentText, node.name, 文件种类);
        }
            ...
            function emitBindingElement(bindingElement: BindingElement) {
                ...
                if (bindingElement.name) {
                    if (isBindingPattern(bindingElement.name)) {
                        ...
                        writeTextOfNode(currentText, bindingElement.name, 文件种类);
            ...
            function emitBindingElement(bindingElement: BindingElement | OmittedExpression) {
                if (bindingElement.kind === SyntaxKind.OmittedExpression) {
                    ...
                    if (bindingElement.propertyName) {
                        ...
                        writeTextOfNode(currentText, bindingElement.propertyName, 文件种类);
                        write(": ");
                    }
                    if (bindingElement.name) {
                        if (isBindingPattern(bindingElement.name)) {
