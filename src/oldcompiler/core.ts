...
namespace ts {
    ...
    export function 创建索引表(): 索引表 {
        return createMap<string>() as 索引表;
    }
   ...
   // Keep the class inside a function so it doesn't get compiled if it's not used.
    ...
    export function 拷贝索引表(map: 索引表): 索引表 {
        const clone = createMap<string>();
        copyEntries(map as Map<string>, clone);
        return clone as 索引表;
    }
    ...
    function getRegularExpressionsForWildcards(specs: ReadonlyArray<string>, basePath: string, usage: "files" | "directories" | "exclude" | "文件集" | "目录集" | "排除"): string[] | undefined {
        ...
        const replaceWildcardCharacter = usage === "files" || usage === "文件集" ? replaceWildCardCharacterFiles : replaceWildCardCharacterOther;
        const singleAsteriskRegexFragment = usage === "files" || usage === "文件集" ? singleAsteriskRegexFragmentFiles : singleAsteriskRegexFragmentOther;

        /**
         * Regex for the ** wildcard. Matches any number of subdirectories. When used for including
         * files or directories, does not match subdirectories that start with a . character
         */
        const doubleAsteriskRegexFragment = usage === "exclude" || usage === "排除" ? "(/.+?)?" : "(/[^/.][^/]*)*?";
    ...
    function getSubPatternFromSpec(spec: string, basePath: string, usage: "files" | "directories" | "exclude" | "文件集" | "目录集" | "排除", singleAsteriskRegexFragment: string, doubleAsteriskRegexFragment: string, replaceWildcardCharacter: (match: string) => string): string | undefined {
        let subpattern = "";
    ...
    export function extensionIsTypeScript(ext: Extension): boolean {
        return ext === Extension.Ts || ext === Extension.CTs || ext === Extension.CTsx || ext === Extension.Tsx || ext === Extension.Dts || ext === Extension.DCts;
    }
