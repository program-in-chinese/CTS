...
namespace ts {
    ...
    function parseJsonConfigFileContentWorker(
        ...
        function getFileNames(): ExpandResult {
            ...
            if (hasProperty(raw, "files") || hasProperty(raw, "文件集")) {
                if (isArray(raw["files"]) || isArray(raw["文件集"])) {

                    fileNames = <string[]>raw["files"] || <string[]>raw["文件集"];
            ...
            if (hasProperty(raw, "include") || hasProperty(raw, "包含")) {
                if (isArray(raw["include"]) || isArray(raw["包含"])) {
                    includeSpecs = <string[]>raw["include"] || <string[]>raw["包含"];
                }
                else {
            ...
            if (hasProperty(raw, "exclude") || hasProperty(raw, "排除")) {
                if (isArray(raw["exclude"]) || isArray(raw["排除"])) {
                    excludeSpecs = <string[]>raw["exclude"] || <string[]>raw["排除"];
                }
                else {
                ...
                let outDir = raw["compilerOptions"] && raw["compilerOptions"]["outDir"];
                if (raw["compilerOptions"]) {
                    outDir = raw["compilerOptions"]["outDir"];
                    if (!outDir) {
                        outDir = raw["compilerOptions"]["输出目录"];
                    }
                }
                if (!outDir && raw["编译选项"]) {
                    outDir = raw["编译选项"]["outDir"];
                    if (!outDir) {
                        outDir = raw["编译选项"]["输出目录"];
                    }
                }
                if (outDir) {
            ...
            if (result.fileNames.length === 0 && (!hasProperty(raw, "files") && resolutionStack.length === 0 && !hasProperty(raw, "文件集") && resolutionStack.length === 0)) {
                errors.push(
    ...
    function parseOwnConfigOfJson(
        ...
        const typeAcquisition = convertTypeAcquisitionFromJsonWorker(json["类型收集"] || json["分类选项"] || json["typeAcquisition"] || json["typingOptions"], basePath, errors, configFileName);
        json.compileOnSave = convertCompileOnSaveOptionFromJson(json, basePath, errors);
        let extendedConfigPath: Path;

        if (json.extends) {
            if (typeof json.extends !== "string") {
                errors.push(createCompilerDiagnostic(Diagnostics.Compiler_option_0_requires_a_value_of_type_1, "排除集", "文字"));
            }
            else {
    ...
    function convertCompileOnSaveOptionFromJson(jsonOption: any, basePath: string, errors: Diagnostic[]): boolean {
        ...
        const result = convertJsonOption(compileOnSaveCommandLineOption, jsonOption["compileOnSave"] || jsonOption["保存时编译"], basePath, errors);
    ...
    function matchFileNames(fileNames: string[], include: string[], exclude: string[], basePath: string, options: CompilerOptions, host: ParseConfigHost, errors: Diagnostic[], extraFileExtensions: JsFileExtensionInfo[], jsonSourceFile: JsonSourceFile): ExpandResult {
        ...
        if (include) {
            include = validateSpecs(include, errors, /*allowTrailingRecursion*/ false, jsonSourceFile, "include", "包含");
        }

        if (exclude) {
            exclude = validateSpecs(exclude, errors, /*allowTrailingRecursion*/ true, jsonSourceFile, "exclude", "排除");
        }
    ...
    function validateSpecs(specs: string[], errors: Diagnostic[], allowTrailingRecursion: boolean, jsonSourceFile: JsonSourceFile, specKey: string, specKey2: string) {
