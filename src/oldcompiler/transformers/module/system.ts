...
namespace ts {
    export function transformSystemModule(context: TransformationContext) {
        ...
        function addExportStarIfNeeded(statements: Statement[]) {
            ...
            for (const externalImport of moduleInfo.externalImports) {
                ...
                for (const element of exportDecl.exportClause.elements) {
                    ...
                    const 名字 = (element.name || element.propertyName).别名 && (element.name || element.propertyName).别名.名称 || (element.name || element.propertyName).text;
                    exportedNames.push(
                        createPropertyAssignment(
                            createLiteral(unescapeLeadingUnderscores(名字)),
                            createTrue()
                        )
                    );
                }
                        ...
                        case SyntaxKind.ExportDeclaration:
                            ...
                            if ((<ExportDeclaration>entry).exportClause) {
                                ...
                                for (const e of (<ExportDeclaration>entry).exportClause.elements) {
                                    properties.push(
                                        createPropertyAssignment(
                                            createLiteral(unescapeLeadingUnderscores(e.name.别名 && e.name.别名.名称 || e.name.text)),
                                            createElementAccess(
                                                parameterName,
                                                createLiteral(unescapeLeadingUnderscores((e.propertyName || e.name).别名 && (e.propertyName || e.name).别名.名称 || (e.propertyName || e.name).text))
                                            )
                                        )
                                    );
                                }
        ...
        function appendExportsOfBindingElement(statements: Statement[] | undefined, decl: VariableDeclaration | BindingElement, exportSelf: boolean): Statement[] | undefined {
            ...
            if (isBindingPattern(decl.name)) {
                ...
                if (exportSelf) {
                    ...
                    excludeName = unescapeLeadingUnderscores(decl.name.别名 && decl.name.别名.名称 || decl.name.text);
                }
        ...
        function appendExportsOfDeclaration(statements: Statement[] | undefined, decl: Declaration, excludeName?: string): Statement[] | undefined {
            ...
            const exportSpecifiers = moduleInfo.exportSpecifiers.get(unescapeLeadingUnderscores(name.别名 && name.别名.名称 || name.text));
            if (exportSpecifiers) {
