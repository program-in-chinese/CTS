...
namespace ts {
    ...
    namespace Parser {
        ...
        let 文件词典组: UnderscoreEscapedMap<词典>;
        let 内置词典: UnderscoreEscapedMap<词典> = 创建内置词典映射();
        let 词典类别: 别名旗帜;
        ...
        function parseSourceFileWorker(fileName: string, languageVersion: ScriptTarget, setParentNodes: boolean, scriptKind: ScriptKind): SourceFile {
            ...
            if (sourceFile.isDeclarationFile) {
                词典类别 = 取词典类别(fileName);
                const 词典语句组 = 编译词典标签.编译全局词典();
                文件词典组 = createUnderscoreEscapedMap<词典>();
                if (词典语句组) {
                    sourceFile.词典语句 = 词典语句组;
                    词典语句组.forEach(v => {
                        if (v && v.表达式) {
                            forEach(v.表达式, 词典 => {
                                if ((词典.flags & NodeFlags.ThisNodeHasError) === 0) {
                                    文件词典组.set(词典.键.name.text as __String, 词典);
                                    if (!词典.是单向词典)  文件词典组.set(词典.值.name.text as __String, 交换词典键值(词典));
                                }
                            });
                        }
                    });
        ...
        let 局部词典组: UnderscoreEscapedMap<词典>;
        ...
        function addJSDocComment<T extends Node>(node: T): T {
            ...
            if (局部字典范围 && 局部字典范围.length > 0) {
                ...
                局部词典组 = createUnderscoreEscapedMap<词典>();
                if (node && node.局部词典语句 && node.局部词典语句.length > 0) {
                    node.局部词典语句.forEach(语句 => {
                        语句.表达式.forEach(v => {
                            局部词典组.set(v.键.name.text as __String, v);
                            if (!v.是单向词典) 局部词典组.set(v.值.name.text as __String, 交换词典键值(v));
                        });
                    });
        ...
        function 回调(node: Node) {
            if (node.kind === SyntaxKind.Identifier) {
                const 子词典 = 局部词典组.get((<Identifier>node).text);
                if (子词典 && (子词典.flags & NodeFlags.ThisNodeHasError) === 0) {
                    const 别名: 别名 = new 别名构造函数(取别名旗帜(子词典), 子词典.值.name.text as __String);
            ...
            if (node.kind === SyntaxKind.StringLiteral) {
                const 子词典 = 局部词典组.get((<StringLiteral>node).text as __String);
        ...
        function createSourceFile(fileName: string, languageVersion: ScriptTarget, scriptKind: ScriptKind): SourceFile {
            ...
            sourceFile.文件种类 = 取文件种类(fileName);
        ...
        function parseExpected(kind: SyntaxKind, diagnosticMessage?: DiagnosticMessage, shouldAdvance = true): boolean {
            ...
            if (diagnosticMessage) {
                ...
                parseErrorAtCurrentToken(Diagnostics._0_expected, 令牌转为中文关键字(kind));
            }
            return false;
        }
        ...
        function createIdentifier(isIdentifier: boolean, diagnosticMessage?: DiagnosticMessage): Identifier {
            ...
            if (isIdentifier) {
                ...
                if (sourceFile.isDeclarationFile && 文件词典组 && node.text) {

                    const 词典值 = 文件词典组.get(node.text); // || 内置词典.get(node.text)
                    if (词典值 && (词典值.flags & NodeFlags.ThisNodeHasError) === 0) {
                        const 别名: 别名 = new 别名构造函数(取别名旗帜(词典值), 词典值.值.name.text as __String);
                        node.别名 = 别名;
                    }
                }
                nextToken();
        ...
        function parsePropertyNameWorker(allowComputedPropertyNames: boolean): PropertyName {
            if (token() === SyntaxKind.StringLiteral || token() === SyntaxKind.NumericLiteral) {
                ...
                if (isStringLiteral(node)) {
                    if (sourceFile.isDeclarationFile && 文件词典组 && 是字面量节点(node) && node.text) {

                        const 词典值 = 文件词典组.get(node.text as __String); // || 内置词典.get(node.text as __String)
        ...
        function parseTemplateSpan(): TemplateSpan {
            ...
            if (token() === SyntaxKind.CloseBraceToken) {
                ...
                literal = <TemplateTail>parseExpectedToken(SyntaxKind.TemplateTail, /*reportAtCurrentPosition*/ false, Diagnostics._0_expected, 令牌转为中文关键字(SyntaxKind.CloseBraceToken));
            }
        ...
        function parseLiteralLikeNode(kind: SyntaxKind): LiteralLikeNode {
            ...
            if (sourceFile.isDeclarationFile && 文件词典组 && 是字面量节点(node) && node.text) {
                const 词典值 = 文件词典组.get(node.text as __String); // || 内置词典.get(node.text as __String)
                if (词典值 && 词典值.是文本字面量词典 &&
                    (词典值.flags & NodeFlags.ThisNodeHasError) === 0) {
                    const 别名: 别名 = new 别名构造函数(取别名旗帜(词典值), 词典值.值.name.text as __String);
                    node.别名 = 别名;
                }
            }
        ...
        function fillSignature(
            ...
            if (returnTokenRequired) {
                ...
                if (backwardToken) {
                    ...
                    signature.type = parseTypeOrTypePredicate();
                    parseErrorAtPosition(start, length, Diagnostics._0_expected, 令牌转为中文关键字(returnToken));
                }
            }
        }
        ...
        function parseConditionalExpressionRest(leftOperand: Expression): Expression {
            ...
            node.colonToken = parseExpectedToken(SyntaxKind.ColonToken, /*reportAtCurrentPosition*/ false,
                Diagnostics._0_expected, 令牌转为中文关键字(SyntaxKind.ColonToken));
        ...
        function parseUnaryExpressionOrHigher(): UnaryExpression | BinaryExpression {
            ...
            if (token() === SyntaxKind.AsteriskAsteriskToken) {
                ...
                if (simpleUnaryExpression.kind === SyntaxKind.TypeAssertionExpression) {
                    ...
                    parseErrorAtPosition(start, simpleUnaryExpression.end - start, Diagnostics.An_unary_expression_with_the_0_operator_is_not_allowed_in_the_left_hand_side_of_an_exponentiation_expression_Consider_enclosing_the_expression_in_parentheses, 令牌转为中文关键字(unaryOperator));
                }
            }
        ...
        function parseJsxElementOrSelfClosingElement(inExpressionContext: boolean): JsxElement | JsxSelfClosingElement {
            ...
            if (opening.kind === SyntaxKind.JsxOpeningElement) {
                ...
                if (!tagNamesAreEquivalent(node.openingElement.tagName, node.closingElement.tagName)) {
                    parseErrorAtPosition(node.closingElement.pos, node.closingElement.end - node.closingElement.pos, Diagnostics.Expected_corresponding_JSX_closing_tag_for_0, getTextOfNodeFromSourceText(sourceText, node.openingElement.tagName, sourceFile.文件种类, 使用场景.类型检查));
                }
        ...
        function parseJsxChildren(openingTagName: LeftHandSideExpression): NodeArray<JsxChild> {
            ...
            while (true) {
                ...
                if (token() === SyntaxKind.LessThanSlashToken) {
                    ...
                    parseErrorAtPosition(openingTagName.pos, openingTagName.end - openingTagName.pos, Diagnostics.JSX_element_0_has_no_corresponding_closing_tag, getTextOfNodeFromSourceText(sourceText, openingTagName, sourceFile.文件种类, 使用场景.类型检查));
                    break;
                }
        ...
        export namespace 编译词典标签 {
            let 当前语境集: NodeFlags;
            let 编译时存在错误: boolean;
            let 词典组: UnderscoreEscapedMap<全局词典语句>;
            let kind: SyntaxKind;
            let 扫描器: Scanner;

            export function 编译局部词典(父节点: Node, start: number, length: number) {
                const saveToken = currentToken;
                const saveParseDiagnosticsLength = parseDiagnostics.length;
                const saveParseErrorBeforeNextFinishedNode = parseErrorBeforeNextFinishedNode;
                const 局部词典 = 编译局部词典语句(start, length);
                if (局部词典) {
                    局部词典.parent = 父节点;
                }
                currentToken = saveToken;
            ...
            export function 编译全局词典(): UnderscoreEscapedMap<全局词典语句> {
                初始化();
                下个();
                while (true) {
                    if (当前的() === SyntaxKind.全局词典语句) {
                        const 语句 = 编译全局词典语句();
                        词典组.set(语句.pos.toString() as __String, 语句);
                    }
                    if (当前的() === SyntaxKind.EndOfFileToken) {
                        break;
                    }
                    下个();
                }
                ...
                置文本("");
                置错误处理( /** onError */ undefined);
                return 词典组.size ? 词典组 : undefined;
            }


            function 编译局部词典语句(start: number, 长度: number) {
                ...
                let 编译键 = false;
                const 表达式: 词典[] = [];
                scanner.scanRange(start + 3, 长度, () => {
                    局部下个();
                    返回值 = <局部词典语句>createNode(SyntaxKind.局部词典语句, scanner.getStartPos());
                    跳出循环: while (true) {
                        switch (token()) {
                            case SyntaxKind.OpenBraceToken:
                ...
                返回值.表达式 = createNodeArray(表达式);
                return finishNode(返回值, 返回值.pos + 长度 - 3);
            }
            function 编译局部表达式(编译键: boolean) {
                let 键, 值;
                let 文本字面量标识符 = false;
                ...
                if (编译键) {
                    if (token() === SyntaxKind.StringLiteral) {
                        ...
                        键.name = 创建局部字面量词典标识符(是局部字面量标识符(), /** 诊断信息 */ undefined);
                        键 = finishNode(键);
                    }
                     else {
                        键 = <词典键>createNode(SyntaxKind.词典键);
                        键.name = 创建局部词典标识符(是局部标识符(), /** 诊断信息 */ undefined);
                        键 = finishNode(键);
                    }
                }
                if (token() === SyntaxKind.ColonToken) {
                    ...
                    if (文本字面量标识符 && token() === SyntaxKind.StringLiteral) {
                        ...
                        值.name = 创建局部字面量词典标识符(是局部字面量标识符(), /** 诊断信息 */ undefined);
                        值 = finishNode(值);
                    }
                     else if (token() === SyntaxKind.StringLiteral) {
                        文本字面量标识符 = false;
                        值 = <词典值>createNode(SyntaxKind.词典值);
                        值.name = 创建局部字面量词典标识符( /** 是字面量标识符 */ false, /** 诊断信息 */ undefined);
                        值 = finishNode(值);
                    }
                    值 = <词典值>createNode(SyntaxKind.词典值);
                    值.name = 创建局部词典标识符(是局部标识符(), /** 诊断信息 */ undefined);
                    值 = finishNode(值);
                }
                if (键 && 值) {
                    返回值.键 = 键;
            ...
            function 创建局部词典标识符(是标识符: boolean, 诊断信息: DiagnosticMessage) {
                if (是标识符) {
                    const 标识符 = <Identifier>createNode(SyntaxKind.Identifier);
                    标识符.text = escapeLeadingUnderscores(internIdentifier(scanner.getTokenValue()));
                    局部下个();
                    return finishNode(标识符);
                }
                局部下个();
                return <Identifier>创建局部词典失踪节点(SyntaxKind.Identifier, 诊断信息 || Diagnostics.有与关键字冲突的词典标识符_名称为_0, scanner.getTokenValue());

            }

            function 创建局部字面量词典标识符(是字面量词典标识符: boolean, 诊断信息: DiagnosticMessage) {
                if (是字面量词典标识符) {
                ...
                局部下个();
                return <StringLiteral>创建局部词典失踪节点(SyntaxKind.StringLiteral, 诊断信息 || Diagnostics.字典的键为文字字面量值必须为文字字面量_错误名称为_0, scanner.getTokenValue());
            ...
            function 发生编译错误(message: DiagnosticMessage, length: number) {
                const pos = 扫描器.getTextPos();
                编译错误在点位(pos, length || 0, message);
            }

            function 初始化() {
                当前语境集 = NodeFlags.词典标签;
                编译时存在错误 = false;
                词典组 = createUnderscoreEscapedMap<全局词典语句>();
                扫描器 = createScanner(sourceFile.languageVersion, /*skipTrivia*/ false, LanguageVariant.Standard, sourceFile.text, 发生编译错误);
                置错误处理(发生编译错误);
            }
            ...
            function 编译全局词典语句() {
                const 语句 = <全局词典语句>创建节点(SyntaxKind.全局词典语句);
                语句.表达式 = 编译词典列表();
                return 完成节点(语句);
            }

            function 创建节点数组<T extends Node>(elements?: T[], pos?: number): NodeArray<T> {
                const array = <NodeArray<T>>(elements || []);
                if (!(pos >= 0)) {
                    pos = 取节点开始();
                }
            ...
            function 创建节点<TKind extends SyntaxKind>(kind: TKind, pos?: number): Node | Token<TKind> | Identifier {
                if (!(pos >= 0)) {
                    pos = 取节点开始();
                }
            ...
            function 完成节点<T extends Node>(node: T, end?: number): T {
                node.end = end === undefined ? 取节点开始() : end;
                if (当前语境集) {
                    node.flags |= 当前语境集;
                }
                if (编译时存在错误) {
            ...
            function 取节点开始() {
                return 扫描器.getStartPos();
            }
            function 取令牌值() {
                return 扫描器.getTokenValue();
            }
            function 置文本(s: string) {
                return 扫描器.setText(s);
            }
            function 置错误处理(onError: ErrorCallback) {
                扫描器.setOnError(onError);
            }
            ...
            function 编译表达式(编译键: boolean) {

                let 键, 值;
                let 文本字面量标识符 = false;
                const 返回值 = <词典>创建节点(SyntaxKind.词典表达式);
                if (编译键) {
                    if (当前的() === SyntaxKind.StringLiteral) {
                        文本字面量标识符 = true;
                        键 = <词典键>创建节点(SyntaxKind.词典键);
                        键.name = 创建字面量标识符(是字面量标识符(), /** 诊断信息 */ undefined);
                        键 = 完成节点(键);
                    }
                    else {
                        键 = <词典键>创建节点(SyntaxKind.词典键);
                        键.name = 创建标识符(是标识符(), /** 诊断信息 */ undefined);
                        键 = 完成节点(键);
                    }
                }
                if (当前的() === SyntaxKind.ColonToken) {
                    下个();
                    if (文本字面量标识符 && 当前的() === SyntaxKind.StringLiteral) {
                        值 = <词典值>创建节点(SyntaxKind.词典值);
                        值.name = 创建字面量标识符(是字面量标识符(), /** 诊断信息 */ undefined);
                        值 = 完成节点(值);
                    }
                    else if (当前的() === SyntaxKind.StringLiteral) {
                        文本字面量标识符 = false;
                        值 = <词典值>创建节点(SyntaxKind.词典值);
                        值.name = 创建字面量标识符(/** 是标识符 */ false, /** 诊断信息 */ undefined);
                        值 = 完成节点(值);
                    }
                    值 = <词典值>创建节点(SyntaxKind.词典值);
                    值.name = 创建标识符(是标识符(), /** 诊断信息 */ undefined);
                    值 = 完成节点(值);
                }
                if (键 && 值) {
                    返回值.键 = 键;
                    ...
                    返回值.词典类别 = 词典类别;
                    while (!(当前的() === SyntaxKind.EndOfFileToken || 当前的() === SyntaxKind.NewLineTrivia || 当前的() === SyntaxKind.CloseBraceToken || 当前的() === SyntaxKind.CommaToken)) {
                        编译时存在错误 = true;
                        下个();
                    }
                    return 完成节点(返回值);
            ...
            function 是标识符() {
                if (当前的() === SyntaxKind.Identifier) return true;
                return 当前的() > SyntaxKind.LastKeyword;
            }
            ...
            function 创建字面量标识符(是标识符: boolean, 诊断信息: DiagnosticMessage) {
                if (是标识符) {
                    const 标识符 = <StringLiteral>创建节点(SyntaxKind.StringLiteral);
                    标识符.text = internIdentifier(取令牌值());
                    下个();
                    const 返回值 = 完成节点(标识符);
                    return 返回值;
                }
                return <StringLiteral>创建失踪节点(SyntaxKind.StringLiteral, 诊断信息 || Diagnostics.字典的键为文字字面量值必须为文字字面量_错误名称为_0, 取令牌值());
            }

            function 创建标识符(是标识符: boolean, 诊断信息: DiagnosticMessage) {
                if (是标识符) {
                    const 标识符 = <Identifier>创建节点(SyntaxKind.Identifier);
                    标识符.text = escapeLeadingUnderscores(internIdentifier(取令牌值()));
                    下个();
                    const 返回值 = 完成节点(标识符);
                    return 返回值;
                }
                return <Identifier>创建失踪节点(SyntaxKind.Identifier, 诊断信息 || Diagnostics.词典值不能为空_请完成词典值的录入);
            }
            function 创建局部词典失踪节点(kind: SyntaxKind, diagnosticMessage: DiagnosticMessage, arg0?: any): Node {
                parseErrorAtPosition(scanner.getStartPos(), 0, diagnosticMessage, arg0);
                const result = createNode(kind, scanner.getStartPos());
                (<Identifier>result).text = "" as __String;
                return finishNode(result);
            ...
            function 创建失踪节点(kind: SyntaxKind, diagnosticMessage: DiagnosticMessage, arg0?: any): Node {
                编译错误在点位(取节点开始(), 0, diagnosticMessage, arg0);
                const result = createNode(kind, 取节点开始());
                kind === SyntaxKind.Identifier ? (<Identifier>result).text = "" as __String : (<StringLiteral>result).text = "";
                return 完成节点(result);
            }

            function 编译错误在点位(start: number, length: number, message: DiagnosticMessage, arg0?: any): void {
                // Don't report another error if it would just be at the same position as the last error.
                const lastError = lastOrUndefined(parseDiagnostics);
                if (!lastError || start !== lastError.start) {
                    parseDiagnostics.push(createFileDiagnostic(sourceFile, start, length, message, arg0));
        ...
        function processReferenceComments(sourceFile: SourceFile): void {
            ...
            while (true) {
                ...
                if (referencePathMatchResult) {
                    ...
                    const amdDependencyRegExCH = /^\/\/\/\s*<AMD附属\s/gim;
                    ...
                    const checkJsDirectiveRegEx = /^\/\/\/?\s*(@ts-check|@ts-nocheck|@cts-检查|@cts-不检查)\s*$/gim;
                    const checkJsDirectiveMatchResult = checkJsDirectiveRegEx.exec(comment);
                    if (checkJsDirectiveMatchResult) {
                        checkJsDirective = {
                            enabled: compareStrings(checkJsDirectiveMatchResult[1], "@ts-check", /*ignoreCase*/ true) === Comparison.EqualTo || compareStrings(checkJsDirectiveMatchResult[1], "@cts-检查", /*ignoreCase*/ true) === Comparison.EqualTo,
                            end: range.end,
                            pos: range.pos
                        };
                    }
        ...
        export namespace JSDocParser {
            ...
            export function parseJSDocComment(parent: Node, start: number, length: number): JSDoc {
                ...
                const saveParseDiagnosticsLength = parseDiagnostics.length;
                const saveParseErrorBeforeNextFinishedNode = parseErrorBeforeNextFinishedNode;
                内置词典 = 创建内置词典映射() as UnderscoreEscapedMap<词典>;
            ...
            export function parseJSDocCommentWorker(start: number, length: number): JSDoc {
                ...
                function parseTypedefTag(atToken: AtToken, tagName: Identifier): JSDocTypedefTag {
                    ...
                    function isObjectTypeReference(node: TypeNode) {
                        return node.kind === SyntaxKind.ObjectKeyword ||
                            isTypeReferenceNode(node) && ts.isIdentifier(node.typeName) && (node.typeName.text === "Object" || node.typeName.text === "基对象类");
                    }
                ...
                function createJSDocIdentifier(isIdentifier: boolean, createIfMissing: boolean): Identifier {
                    ...
                    const 词典值 = 文件词典组 && 文件词典组.get(result.text); // || 文件词典组 && 内置词典.get(result.text)
                    if (词典值) {
                        const 别名: 别名 = new 别名构造函数(取别名旗帜(词典值), 词典值.值.name.text as __String);
                        result.别名 = 别名;
                    }
