...
namespace ts {
    ...
    const enum ContainerFlags {
        ...
        // recursing into it.
        // 当前节点不是一个容器，没有容器操作应该在检索到它发生。
        None = 0,

        // The current node is a container.  It should be set as the current container (and block-
        // container) before recursing into it.  The current node does not have locals.  Examples:
        // 当前节点是一个容器。它应该被设置为当前的容器（和块容器）之前检索到它。当前节点没有当地人。实例：
        //      Classes, ObjectLiterals, TypeLiterals, Interfaces...
        IsContainer = 1 << 0,

        // The current node is a block-scoped-container.  It should be set as the current block-
        // container before recursing into it.  Examples:
        // 当前节点是一块范围内的容器。它应该被设置为当前块在检索到它的容器。实例：
        //      Blocks (when not parented by functions), Catch clauses, For/For-in/For-of statements...
        IsBlockScopedContainer = 1 << 1,

        // The current node is the container of a control flow path. The current control flow should
        // be saved and restored, and a new control flow initialized within the container.
        // 当前节点是一个控制流路径的容器。当前的控制流应该被保存和恢复，和一个新的控制流初始化容器内。
    ...
    function createBinder(): (file: SourceFile, options: CompilerOptions) => void {
        ...
        function bindSourceFile(f: SourceFile, opts: CompilerOptions) {
            file = f;
            options = opts;
            生成符号 = options.转译声明 ? file.文件种类 === 文件种类.DCTS || file.文件种类 === 文件种类.DTS : file.文件种类 === 文件种类.DCTS || file.文件种类 === 文件种类.DTS;
        ...
        function 取节点别名(node: Declaration): 别名 {
            if (!生成符号) {
                return undefined;
            }
            ...
            if (name) {
                if (isAmbientModule(node)) {
                    return (<Identifier | LiteralExpression>name).别名;
                }
                if (name.kind === SyntaxKind.ComputedPropertyName) {
                    const nameExpression = (<ComputedPropertyName>name).expression;
                    // treat computed property names where expression is string/numeric literal as just string/numeric literal
                    if (isStringLiteral(nameExpression)) {
                        return nameExpression.别名;
                    }
                    Debug.assert(isWellKnownSymbolSyntactically(nameExpression));
                    return (<PropertyAccessExpression>nameExpression).name.别名;
                }
                return (<Identifier | LiteralExpression>name).别名;
            }
            switch (node.kind) {
                    ...
                    if (parentNode && parentNode.kind === SyntaxKind.VariableStatement) {
                        if ((<VariableStatement>parentNode).declarationList.declarations.length > 0) {
                            ...
                            if (nameIdentifier.kind === SyntaxKind.Identifier) {
                                return (<Identifier>nameIdentifier).别名;
                            }
                        }
                    }
                    return;
        ...
        function createSymbol(flags: SymbolFlags, name: __String, 别名参数?: 别名): Symbol {
            symbolCount++;
            const 符号值 = new Symbol(flags, name);
            if (别名参数) {
                符号值.别名 = 别名参数;
            }
            return 符号值;
        ...
        function declareSymbol(symbolTable: SymbolTable, parent: Symbol, node: Declaration, includes: SymbolFlags, excludes: SymbolFlags): Symbol {
            ...
            const 别名参数 = isDefaultExport && parent ? undefined : 取节点别名(node);
            ...
            if (name === undefined) {
                ...
                if (!symbol) {
                    symbolTable.set(name, symbol = createSymbol(SymbolFlags.None, name, 别名参数));
                }
                ...
                if (name && (includes & SymbolFlags.Classifiable)) {
                    ...
                    if (别名参数 && 别名参数.名称) {
                        ...
                        classifiableNames.set(别名参数.名称, true);
                ...
                if (symbol.flags & excludes) {
                    if (symbol.isReplaceableByMethod) {
                        ...
                        symbol = createSymbol(SymbolFlags.None, name, 别名参数);
                    }
                }
            }
        ...
        function bindFunctionOrConstructorType(node: SignatureDeclaration): void {
            ...
            const symbol = createSymbol(SymbolFlags.Signature, getDeclarationName(node), 取节点别名(node));
        ...
        function bindAnonymousDeclaration(node: Declaration, symbolFlags: SymbolFlags, name: __String, 别名参数?: 别名) {
            const symbol = createSymbol(symbolFlags, name, 别名参数);
        ...
        function isEvalOrArgumentsIdentifier(node: Node): boolean {
            return node.kind === SyntaxKind.Identifier &&
                ((<Identifier>node).text === "eval" || (<Identifier>node).text === "执行" || (<Identifier>node).text === "arguments" || (<Identifier>node).text === "增强参数集");
        }
        ...
        function isUseStrictPrologueDirective(node: ExpressionStatement): boolean {
            const nodeText = getTextOfNodeFromSourceText(file.text, node.expression, file.文件种类, 使用场景.输出);

            // Note: the node text must be exactly "use strict" or 'use strict'.  It is not ok for the
            // string to contain unicode escapes (as per ES5).
            return nodeText === '"use strict"' || nodeText === "'use strict'" || nodeText === '"使用严格模式"' || nodeText === "'使用严格模式'";
        }
        ...
        function bindSourceFileAsExternalModule() {
            bindAnonymousDeclaration(file, SymbolFlags.ValueModule, `"${removeFileExtension(file.fileName)}"` as __String, /***/ undefined);
        }

        function bindExportAssignment(node: ExportAssignment | BinaryExpression) {
            if (!container.symbol || !container.symbol.exports) {
                // Export assignment in some sort of block construct
                bindAnonymousDeclaration(node, SymbolFlags.Alias, getDeclarationName(node), 取节点别名(node));
            }
            else {
        ...
        function bindExportDeclaration(node: ExportDeclaration) {
            if (!container.symbol || !container.symbol.exports) {
                ...
                bindAnonymousDeclaration(node, SymbolFlags.ExportStar, getDeclarationName(node), 取节点别名(node));
            }
        ...
        function bindThisPropertyAssignment(node: BinaryExpression) {
            ...
            switch (container.kind) {
                ...
                case SyntaxKind.SetAccessor:
                    ...
                    const symbol = declareSymbol(hasModifier(container, ModifierFlags.Static) ? containingClass.symbol.exports :
                        containingClass.symbol.members,
                        containingClass.symbol, node, SymbolFlags.Property, SymbolFlags.None);
                    if (symbol) {
        ...
        function bindClassLikeDeclaration(node: ClassLikeDeclaration) {
            if (node.kind === SyntaxKind.ClassDeclaration) {
                ...
                const 别名参数 = node.name ? node.name.别名 : undefined;
                bindAnonymousDeclaration(node, SymbolFlags.Class, bindingName, 别名参数);
        ...
        function bindParameter(node: ParameterDeclaration) {
            ...
            if (isBindingPattern(node.name)) {
                bindAnonymousDeclaration(node, SymbolFlags.FunctionScopedVariable, "__" + indexOf(node.parent.parameters, node) as __String, node.name.别名);
            }
            else {
                declareSymbolAndAddToSymbolTable(node, SymbolFlags.FunctionScopedVariable, SymbolFlags.ParameterExcludes);
            }
        ...
        function bindFunctionExpression(node: FunctionExpression) {
            ...
            const 别名参数 = node.name ? node.name.别名 : undefined;
            return bindAnonymousDeclaration(node, SymbolFlags.Function, bindingName, 别名参数);
        }
    ...
    function computeParenthesizedExpression(node: ParenthesizedExpression, subtreeFlags: TransformFlags) {
        ...
        // there.
        // 如果节点是合成的，这意味着发射把括号里，而不是用户。如果我们不想要这些东西，发射器不会把它们放在那里。
        ...
        // 如果一个括号表达式的表达式是一个解构的任务，然后用括号括起来的表达式是一个解构的任务。
    ...
    function computeOther(node: Node, kind: SyntaxKind, subtreeFlags: TransformFlags) {
        ...
        switch (kind) {
            ...
            case SyntaxKind.StringLiteral:
                ...
                /*
                let 父节点 = node.parent
                if (父节点 && isCallExpression(父节点)) {
                    transformFlags |= TransformFlags.CTS转换
                }
                */
                break;
     ...
     * 获取转换标志来排除时，结合变换旗帜的子树。
     * NOTE: This needs to be kept up-to-date with the exclusions used in `computeTransformFlagsForNode`.
     *       For performance reasons, `computeTransformFlagsForNode` uses local constant values rather
     *       than calling this function.
     * 注意: 这需要保持最新的与用于` computetransformflagsfornode `拒赔。出于性能原因，
     *      ` computetransformflagsfornode `采用局部常值而不是调用这个函数。
     */
    /* @internal */
