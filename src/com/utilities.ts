...
/// <reference path="工具.ts"/>

/* @internal */
namespace ts {

    export function 写调试记录(数据: string) {
        const 路径 = combinePaths(sys.取系统临时文件目录(), "CHTypescript");
        const 文件名 = "CTS_调试.txt";
        if (!sys.directoryExists(路径)) {
            sys.createDirectory(路径);
        }
        sys.appendFile(combinePaths(路径, 文件名), 数据);
    }
    ...
    export function getSourceTextOfNodeFromSourceFile(sourceFile: SourceFile, node: Node, 场景 = 使用场景.输出, includeTrivia = false): string {
        if (nodeIsMissing(node)) {
            return "";
        }

        return getTextOfNodeFromSourceText(sourceFile.text, node, sourceFile.文件种类 || 取文件种类(sourceFile.fileName), 场景, includeTrivia);
    }


    export function getTextOfNodeFromSourceText(sourceText: string, node: Node, 文件种类: 文件种类, 场景 = 使用场景.类型检查, 包含琐事 = false): string {
        if (nodeIsMissing(node)) {
            return "";
        }
        return 取输出文本(sourceText, node, 文件种类, 场景, 包含琐事);
    }

    export function getTextOfNode(node: Node, includeTrivia = false): string {
        return getSourceTextOfNodeFromSourceFile(getSourceFileOfNode(node), node, 使用场景.类型检查, includeTrivia);
    }
    ...
    export function getLiteralText(node: LiteralLikeNode, sourceFile: SourceFile) {
        ...
        const 输出文本 = node.别名 ? 输出节点编码英文(node) as string : node.text;

        // If we can't reach the original source text, use the canonical form if it's a number,
        // or a (possibly escaped) quoted form of the original text if it's string-like.
        switch (node.kind) {
            case SyntaxKind.StringLiteral:
                return '"' + escapeText(输出文本) + '"';
    ...
    export function entityNameToString(name: EntityNameOrEntityNameExpression): string {
        switch (name.kind) {
            case SyntaxKind.Identifier:
                return getFullWidth(name) === 0 ? unescapeLeadingUnderscores(输出节点编码中文(name) as __String) : getTextOfNode(name);
    ...
    export let fullTripleSlashReferencePathRegExCH = /^(\/\/\/\s*<引用\s+路径\s*=\s*)('|")(.+?)\2.*?\/>/;

    export let fullTripleSlashReferenceTypeReferenceDirectiveRegEx = /^(\/\/\/\s*<reference\s+types\s*=\s*)('|")(.+?)\2.*?\/>/;
    export let fullTripleSlashReferenceTypeReferenceDirectiveRegExCH = /^(\/\/\/\s*<引用\s+类型集\s*=\s*)('|")(.+?)\2.*?\/>/;

    export let fullTripleSlashAMDReferencePathRegEx = /^(\/\/\/\s*<amd-dependency\s+path\s*=\s*)('|")(.+?)\2.*?\/>/;
    export let fullTripleSlashAMDReferencePathRegExCH = /^(\/\/\/\s*<AMD附属\s+路径\s*=\s*)('|")(.+?)\2.*?\/>/;
    ...
    export function isRequireCall(callExpression: Node, checkArgumentIsStringLiteral: boolean): callExpression is CallExpression {
        ...
        if (expression.kind !== SyntaxKind.Identifier || ((expression as Identifier).text !== "require" && (expression as Identifier).text !== "需要")) {
            return false;
        }
    ...
    export function isExportsIdentifier(node: Node) {
        return isIdentifier(node) && (node.text === "exports" || node.text === "导出集");
    }

    export function isModuleExportsPropertyAccessExpression(node: Node) {
        return isPropertyAccessExpression(node) && isIdentifier(node.expression) && (node.expression.text === "module" || node.expression.text === "模块") && (node.name.text === "exports" || node.name.text === "导出集");
    }
    ...
    export function getSpecialPropertyAssignmentKind(expression: ts.BinaryExpression): SpecialPropertyAssignmentKind {
        ...
        if (lhs.expression.kind === SyntaxKind.Identifier) {
            ...
            if (lhsId.text === "exports" || lhsId.text === "导出集") {
                // exports.name = expr
                return SpecialPropertyAssignmentKind.ExportsProperty;
            }
            else if (lhsId.text === "module" || lhsId.text === "模块" && lhs.name.text === "exports" || lhs.name.text === "导出集") {
                // module.exports = expr
            ...
            if (innerPropertyAccess.expression.kind === SyntaxKind.Identifier) {
                ...
                if (innerPropertyAccessIdentifier.text === "module" || innerPropertyAccessIdentifier.text === "模块" && innerPropertyAccess.name.text === "导出集" || innerPropertyAccess.name.text === "exports") {
                    return SpecialPropertyAssignmentKind.ExportsProperty;
                }
                if (innerPropertyAccess.name.text === "prototype" || innerPropertyAccess.name.text === "原型") {
    ...
    export function isJSDocConstructSignature(node: Node) {
        return node.kind === SyntaxKind.JSDocFunctionType &&
            ...
            (((node as JSDocFunctionType).parameters[0].name as Identifier).text === "new" || ((node as JSDocFunctionType).parameters[0].name as Identifier).text === "新建");
    }
    ...
    export function getJSDocParameterTags(param: ParameterDeclaration): JSDocParameterTag[] | undefined {
        ...
        if (!param.name) {
            ...
            return tags.filter((tag): tag is JSDocParameterTag => isJSDocParameterTag(tag) && (tag.name.text === name || tag.name.别名 && tag.name.别名.名称 === name)) as JSDocParameterTag[];
        }
        else {
    ...
    export function getParameterFromJSDoc(node: JSDocParameterTag): ParameterDeclaration | undefined {
        ...
        return find(grandParent.parameters, p =>
            p.name.kind === SyntaxKind.Identifier && (p.name.text === name || p.name && p.name.别名 && p.name.别名.名称 === name));
    }
    ...
    export function getTypeParameterFromJsDoc(node: TypeParameterDeclaration & { parent: JSDocTemplateTag }): TypeParameterDeclaration | undefined {
        ...
        return find(typeParameters, p => p.name.text === name || p.name && p.name.别名 && p.name.别名.名称 === name);
    }
    ...
    export function getFileReferenceFromReferencePath(comment: string, commentRange: CommentRange): ReferencePathMatchResult {
        ...
        const isNoDefaultLibRegExCH = /^(\/\/\/\s*<引用\s+不使用默认支持库\s*=\s*)('|")(.+?)\2\s*\/>/gim;
    ...
    export function 取属性别名从属性声明(name: DeclarationName): __String {
        if (name.kind === SyntaxKind.Identifier) {
            return (取别名名称(name) as __String) || name.text as __String;
        }
        if (name.kind === SyntaxKind.StringLiteral) {
            return escapeLeadingUnderscores(取别名名称(name) as string) || escapeLeadingUnderscores(name.text);
        }
        if (name.kind === SyntaxKind.ComputedPropertyName) {
            const nameExpression = name.expression;
            if (isWellKnownSymbolSyntactically(nameExpression)) {
                const rightHandSideName = 取别名名称((<PropertyAccessExpression>nameExpression).name) || (<PropertyAccessExpression>nameExpression).name.text;
                return getPropertyNameForKnownSymbolName(unescapeLeadingUnderscores(rightHandSideName as __String));
            }
            else if (nameExpression.kind === SyntaxKind.StringLiteral || nameExpression.kind === SyntaxKind.NumericLiteral) {
                return escapeLeadingUnderscores((取别名名称((<LiteralExpression>nameExpression)) as string) || (<LiteralExpression>nameExpression).text);
            }
        }
    ...
    export function getTextOfIdentifierOrLiteral(node: Identifier | LiteralLikeNode) {
        if (node) {
            if (node.kind === SyntaxKind.Identifier) {
                return ((node as Identifier).text as string);
            }
            if (node.kind === SyntaxKind.StringLiteral) {
                return ((node as LiteralLikeNode).text as string);
            }
    ...
    export function 取标识符或字面量的文本(node: Identifier | LiteralLikeNode) {
        if (node) {
            if (node.kind === SyntaxKind.Identifier || node.kind === SyntaxKind.StringKeyword) {
                return 输出节点编码中文(node) || getTextOfIdentifierOrLiteral(node)
            }
            return getTextOfIdentifierOrLiteral(node)
        }
        return undefined
    }
    ...
    */
    ...
    export function 取标识符或字面量编码文本(node: Identifier | LiteralLikeNode) {
        if (node) {
            if (node.kind === SyntaxKind.Identifier) {
                return 取符号节点类型中文((node as Identifier)) as __String;
            }
            if (node.kind === SyntaxKind.StringLiteral ||
                node.kind === SyntaxKind.NumericLiteral) {
                return escapeLeadingUnderscores((node as LiteralLikeNode).text);
    ...
    export function isESSymbolIdentifier(node: Node): boolean {
        return node.kind === SyntaxKind.Identifier && ((<Identifier>node).text === "Symbol" || (<Identifier>node).text === "符号类");
    }

    export function isPushOrUnshiftIdentifier(node: Identifier) {
        return node.text === "push" || node.text === "压入" || node.text === "unshift" || node.text === "添加首元素";
    }
    ...
    export function createTextWriter(newLine: string): EmitTextWriter {
        ...
        function writeTextOfNode(text: string, node: Node, 文件种类: 文件种类) {
            write(getTextOfNodeFromSourceText(text, node, 文件种类, 使用场景.输出));
        }
    ...
    export function getSourceFilesToEmit(host: EmitHost, targetSourceFile?: SourceFile): SourceFile[] {
        ...
        if (options.转译声明) {
            const 声明文件集 = targetSourceFile === undefined ? host.getSourceFiles() : [targetSourceFile];
            return filter(声明文件集, 文件 => 筛选(文件));  // 取文件种类(targetSourceFile.fileName) === 文件种类.DTS && [targetSourceFile]
        }
    ...
    export function 筛选(文件: SourceFile) {
        return 文件 && 取文件种类(文件.fileName) === 文件种类.DTS;
    }
    ...
    export function createTokenRange(pos: number, token: SyntaxKind): TextRange {
        return createRange(pos, pos + 令牌转为关键字(token).length);
    }
    ...
    export function isWatchSet(options: CompilerOptions) {
        ...
        return options.watch && options.hasOwnProperty("watch") || options.hasOwnProperty("监控");
    }
...
namespace ts {
    ...
    export function 跳过括号表达式(node: Expression) {
