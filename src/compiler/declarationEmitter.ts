...
namespace ts {
    ...
    function emitDeclarations(host: EmitHost, resolver: EmitResolver, emitterDiagnostics: DiagnosticCollection, declarationFilePath: string,
        ...
        // const 使用中文关键字 = compilerOptions.useUnicodeKeywords || false;
        const 不转义字符 = compilerOptions.unescapedUnicode || false;
        ...
        function emitEnumMemberDeclaration(node: EnumMember) {
            ...
            if (enumMemberValue !== undefined) {
                write(" = ");
                write(getTextOfConstantValue(enumMemberValue, 不转义字符));
            }
            write(",");
            writeLine();
        }
