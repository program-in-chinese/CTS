...
namespace ts {
    ...
    export function createSynthesizedIdentifier(text: string): Identifier {
    ...
    export function createSynthesizedNodeArray<T extends Node>(elements?: ReadonlyArray<T>, hasTrailingComma?: boolean): NodeArray<T> {
    ...
    namespace Parser {
        ...
        export function parseJsonText(fileName: string, sourceText: string): JsonSourceFile {
            ...
            sourceFile = createSourceFile(fileName, ScriptTarget.ES2015, ScriptKind.JSON, /*isDeclaration*/ false, /*isCtsDeclaration*/ false);
        ...
        function parseSourceFileWorker(fileName: string, languageVersion: ScriptTarget, setParentNodes: boolean, scriptKind: ScriptKind): SourceFile {
            const isCtsDeclarationFile = isCtsDeclarationFileName(fileName);
            const isDeclarationFile = isCtsDeclarationFile || isDeclarationFileName(fileName);

            if (isDeclarationFile) {
                contextFlags |= NodeFlags.Ambient;
            }

            sourceFile = createSourceFile(fileName, languageVersion, scriptKind, isDeclarationFile, isCtsDeclarationFile);
            ...
            sourceFile.endOfFileToken = 添加词典(addJSDocComment(parseTokenNode()));
        ...
        function 添加词典<T extends Node>(node: T): T {
            if (sourceFile.isCtsDeclarationFile) {
                const 词典注释 = 取字典注释范围(node, sourceFile.text);
                if (词典注释) {
                    for (const 注释 of 词典注释) {
                        const 全局语句 = 编译词典(注释.pos, 注释.end, sourceFile);
                        if (全局语句 && 全局语句.表达式) {
                            if (!sourceFile.全局词典) {
        ...
        function 编译词典(开始: number, 结尾: number, 源码: SourceFile) {
            const 词典文本 = 源码.text.substring(开始, 结尾);
            const 词典体 = 词典文本.match(/\/\/@@{(.+)?}/) || [];
            let 头长度 = 开始 + 5;
            if (词典体.length = 2) {
                const 词典内容 = 词典体[1];
                const 键值对 = 词典内容.split(",");
                const 表达式数组 = [];
                for (const v of 键值对) {
                    const 键值 = v.split(":");
                    const 键文本 = 键值[0].trim();
                    const 值文本 = 键值[1].trim();
                    const chjt = 键文本.charCodeAt(0);
                    const chjw = 键文本.charCodeAt(键文本.length - 1);

                    const chzt = 值文本.charCodeAt(0);
                    const chzw = 值文本.charCodeAt(值文本.length - 1);

                    const 词典表达式 = ts.createNode(SyntaxKind.词典表达式, 头长度, 头长度 += v.length) as 词典;

                    if (chjt === CharacterCodes.doubleQuote && chjw === CharacterCodes.doubleQuote || chjt === CharacterCodes.singleQuote && chjw === CharacterCodes.singleQuote) {
                        // 键是字符串
                        if (chzt === CharacterCodes.doubleQuote && chzw === CharacterCodes.doubleQuote || chzt === CharacterCodes.singleQuote && chzw === CharacterCodes.singleQuote) {
                            const 键名 = ts.createStringLiteral(键文本.slice(1, -1));
                            const 值名 = ts.createStringLiteral(值文本.slice(1, -1));
                            if (!键名 || !值名) {
                                parseErrorBeforeNextFinishedNode = true;
                                break;
                            }
                            const 词典键 = ts.createNode(SyntaxKind.词典键, -1, -1) as 词典键;
                            词典键.name = 键名;
                            const 词典值 = ts.createNode(SyntaxKind.词典值, -1, -1) as 词典值;
                            词典值.name = 值名;
                            词典表达式.键 = 词典键;
                            词典表达式.值 = 词典值;
                            词典表达式.是文本字面量词典 = 别名旗帜.字面量;
                            词典表达式.词典类别 = 别名旗帜.汉英;
                        }
                        else {
                            parseErrorBeforeNextFinishedNode = true;
                    ...
                    else if (isIdentifierText(键文本, 源码.languageVersion) && isIdentifierText(值文本, 源码.languageVersion)) {
                        const 键名 = ts.createSynthesizedIdentifier(键文本);
                        const 值名 = ts.createSynthesizedIdentifier(值文本);
                        const 词典键 = ts.createNode(SyntaxKind.词典键, -1, -1) as 词典键;
                        词典键.name = 键名;
                        const 词典值 = ts.createNode(SyntaxKind.词典值, -1, -1) as 词典值;
                        词典值.name = 值名;
                ...
                if (parseErrorBeforeNextFinishedNode) {
                    parseErrorAtPosition(开始, 结尾 - 开始, Diagnostics.字典语句包含错误);
                    const 返回值 = ts.createNode(SyntaxKind.词典语句, 开始, 结尾) as 词典语句;
                    return finishNode(返回值, 结尾);
                }
                const 返回值 = ts.createNode(SyntaxKind.词典语句, 开始, 结尾) as 词典语句;
                返回值.表达式 = ts.createSynthesizedNodeArray(表达式数组);
                return finishNode(返回值, 结尾);
            }
            parseErrorAtPosition(开始, 结尾 - 开始, Diagnostics.字典语句包含错误);
            const 返回值 = ts.createNode(SyntaxKind.词典语句, 开始, 结尾) as 词典语句;
        ...
        function createSourceFile(fileName: string, languageVersion: ScriptTarget, scriptKind: ScriptKind, isDeclarationFile: boolean, isCtsDeclarationFile: boolean): SourceFile {
            ...
            sourceFile.isCtsDeclarationFile = isCtsDeclarationFile;
        ...
        function parseSimpleArrowFunctionExpression(identifier: Identifier, asyncModifier?: NodeArray<Modifier>): ArrowFunction {
            ...
            addJSDocComment(finishNode(node));
            return 添加词典(node);
        }
        ...
        export namespace JSDocParser {
            export function parseJSDocTypeExpressionForTests(content: string, start: number, length: number): { jsDocTypeExpression: JSDocTypeExpression, diagnostics: Diagnostic[] } | undefined {
                ...
                sourceFile = createSourceFile("file.js", ScriptTarget.Latest, ScriptKind.JS, /*isDeclarationFile*/ false, /*isCtsDeclarationFile*/ false);
            ...
            export function parseJSDocCommentWorker(start: number, length: number): JSDoc {
                ...
                function parseTag(indent: number) {
                    ...
                    if (tagName) {
                        switch (tagName.escapedText) {
                            case "augments":
                            case "extends":
                            case unicodeDic.JSDoc.augments:
                            case unicodeDic.JSDoc.extends:
                                tag = parseAugmentsTag(atToken, tagName);
                                break;
                            case "class":
                            case "constructor":
                            case unicodeDic.JSDoc.class:
                            case unicodeDic.JSDoc.constructor:
                                tag = parseClassTag(atToken, tagName);
                                break;
                            case "arg":
                            case "argument":
                            case "param":
                            case unicodeDic.JSDoc.arg:
                            case unicodeDic.JSDoc.argument:
                            case unicodeDic.JSDoc.param:
                                tag = parseParameterOrPropertyTag(atToken, tagName, PropertyLikeParse.Parameter);
                                break;
                            case "return":
                            case "returns":
                            case unicodeDic.JSDoc.return:
                            case unicodeDic.JSDoc.returns:
                                tag = parseReturnTag(atToken, tagName);
                                break;
                            case "template":
                            case unicodeDic.JSDoc.template:
                                tag = parseTemplateTag(atToken, tagName);
                                break;
                            case "type":
                            case unicodeDic.JSDoc.type:
                                tag = parseTypeTag(atToken, tagName);
                                break;
                            case "typedef":
                            case unicodeDic.JSDoc.typedef:
                ...
                function tryParseChildTag(target: PropertyLikeParse): JSDocTypeTag | JSDocPropertyTag | JSDocParameterTag | false {
                    ...
                    switch (tagName.escapedText) {
                        ...
                        case "prop":
                        case unicodeDic.JSDoc.prop:
                        case "property":
                        case unicodeDic.JSDoc.property:
    ...
    function isCtsDeclarationFileName(fileName: string): boolean {
        return fileExtensionIs(fileName, Extension.DCts);
    }
    ...
    function extractPragmas(pragmas: PragmaPsuedoMapEntry[], range: CommentRange, text: string) {
        ...
        if (tripleSlash) {
            let name = tripleSlash[1].toLowerCase() as keyof PragmaPsuedoMap; // Technically unsafe cast, but we do it so the below check to make it safe typechecks
            /** [__`编译指示`__](#翻译) */
            if ((name as string) === unicodeDic.TripleSlash.reference) {
                name = "reference";
            }
            else if ((name as string) === unicodeDic.TripleSlash.amd_dependency) {
                name = "amd-dependency";
            }
            else if ((name as string) === unicodeDic.TripleSlash.amd_module) {
                name = "amd-module";
            }
            else if ((name as string) === unicodeDic.TripleSlash.ts_check) {
                name = "ts-check";
            }
            else if ((name as string) === unicodeDic.TripleSlash.ts_nocheck) {
                name = "ts-nocheck";
            }
            else if ((name as string) === unicodeDic.TripleSlash.jsx) {
                name = "jsx";
            }
            ...
            if (pragma.args) {
                ...
                for (const arg of pragma.args) {
                    let nameUn = "";
                    if (arg.name === "types") {
                        nameUn = unicodeDic.TripleSlash.types;
                    }
                    else if (arg.name === "path") {
                        nameUn = unicodeDic.TripleSlash.path;
                    }
                    else if (arg.name === "no-default-lib") {
                        nameUn = unicodeDic.TripleSlash.no_default_lib;
                    }
                    else if (arg.name === "name") {
                        nameUn = unicodeDic.TripleSlash.name;
                    }
                    else if (arg.name === "factory") {
                        nameUn = unicodeDic.TripleSlash.factory;
                    }

                    const matcher = getNamedArgRegEx(arg.name);
                    const matcherUn = getNamedArgRegEx(nameUn);
                    const matchResult = matcher.exec(text) || matcherUn.exec(text);
    ...
    function addPragmaForMatch(pragmas: PragmaPsuedoMapEntry[], range: CommentRange, kind: PragmaKindFlags, match: RegExpExecArray) {
        if (!match) return;
        let name = match[1].toLowerCase() as keyof PragmaPsuedoMap; // Technically unsafe cast, but we do it so they below check to make it safe typechecks
        if ((name as string) === unicodeDic.TripleSlash.reference) {
            name = "reference";
        }
        else if ((name as string) === unicodeDic.TripleSlash.amd_dependency) {
