...
namespace ts.server {
    ...
    export class Session implements EventSender {
        ...
        private transformCts(req: protocol.FileRequestArgs): string {
            const { project } = this.getFileAndProject(req);
            return project.getLanguageService().转为CTS(req.file);
        }

        private formattedDictionary(req: protocol.FileRequestArgs): string {
            const { project } = this.getFileAndProject(req);
            return project.getLanguageService().格式化词典语句(req.file);
        }

        private dictionaryCompletion(args: protocol.词典完成请求参数): protocol.词典完成条目 {
            const file = toNormalizedPath(args.file);
            const projects = this.getProjects(args);
            const position = this.getPositionInFile(args, file);
            const ignoreName = args.ignoreName;

            const 词典信息 = combineProjectOutput(file,
                path => this.projectService.getScriptInfoForPath(path).fileName, projects,
                p => {
                    const 信息 = p.getLanguageService().取词典自动完成项目新(file, position, ignoreName);
                    if (!信息) {
                        return emptyArray;
                    }
            ...
            for (let i = 0; i < 词典信息.length; i++) {
                ...
                if (i === 0) {
                    ...
                    const 存在 = body.rangeMap[当前.file];
                    if (存在) {
        ...
        private handlers = createMapFromTemplate<(request: protocol.Request) => HandlerResponse>({
            ...
            [CommandNames.transformCts]: (request: protocol.FileRequest) => {
                return this.requiredResponse(this.transformCts(request.arguments));
            },
            [CommandNames.formattedDictionary]: (request: protocol.FileRequest) => {
                return this.requiredResponse(this.formattedDictionary(request.arguments));
            },
            [CommandNames.dictionaryCompletion]: (request: protocol.词典完成请求) => {
                return this.requiredResponse(this.dictionaryCompletion(request.arguments));
            }
        });
