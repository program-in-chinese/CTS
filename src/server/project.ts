...
namespace ts.server {
    ...
    export function countEachFileTypes(infos: ScriptInfo[]): FileStats {
        const result = { js: 0, jsx: 0, ts: 0, cts: 0, tsx: 0, ctsx: 0, dts: 0, dcts: 0 };
        for (const info of infos) {
            switch (info.scriptKind) {
                ...
                case ScriptKind.TS:
                    ...
                    break;
                case ScriptKind.CTS:
                    fileExtensionIs(info.fileName, Extension.DCts)
                        ? result.dcts += 1
                        : result.cts += 1;
                    break;
                case ScriptKind.TSX:
    ...
    function hasOneOrMoreJsAndNoTsFiles(project: Project) {
        ...
        return counts.js > 0 && counts.ts === 0 && counts.cts === 0 && counts.tsx === 0 && counts.ctsx === 0;
    }

    export function allRootFilesAreJsOrDts(project: Project): boolean {
        const counts = countEachFileTypes(project.getRootScriptInfos());
        return counts.ts === 0 && counts.cts === 0 && counts.tsx === 0 && counts.ctsx === 0;
    }
    ...
    export function hasNoTypeScriptSource(fileNames: string[]): boolean {
        return !fileNames.some(fileName => (fileExtensionIs(fileName, Extension.Ts) && !fileExtensionIs(fileName, Extension.Dts)) || fileExtensionIs(fileName, Extension.CTs) && fileExtensionIs(fileName, Extension.DCts) || fileExtensionIs(fileName, Extension.Tsx) || fileExtensionIs(fileName, Extension.CTsx));
    }
