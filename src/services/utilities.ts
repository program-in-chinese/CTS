...
namespace ts {
    ...
    export function displayPart(text: string, kind: SymbolDisplayPartKind): SymbolDisplayPart {
        return { text: textOrKeyword(text), kind: SymbolDisplayPartKind[kind] };
    }
    export function displayPart_2(text: string, kind: SymbolDisplayPartKind): SymbolDisplayPart {
        return { text, kind: SymbolDisplayPartKind[kind] };
    }
    
    export function spacePart() {
        return displayPart_2(" ", SymbolDisplayPartKind.space);
    }

    export function keywordPart(kind: SyntaxKind) {
        return displayPart_2(tokenToStringUn(kind), SymbolDisplayPartKind.keyword);
    }

    export function punctuationPart(kind: SyntaxKind) {
        return displayPart_2(tokenToStringUn(kind), SymbolDisplayPartKind.punctuation);
    }

    export function operatorPart(kind: SyntaxKind) {
        return displayPart_2(tokenToStringUn(kind), SymbolDisplayPartKind.operator);
    }

    export function textOrKeyword(text: string) {
        const kind = stringToToken(text) || stringToTokenUn(text);
        return kind === undefined ? text : tokenToStringUn(kind)
    }
