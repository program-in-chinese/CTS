/* @internal */
namespace ts.词典完成新 {
    const 忽略的名称映射: Map<boolean> = createMap();
    ...
    export function 取词典自动完成项目新(程序集: Program, 取消: CancellationToken, sourceFile: SourceFile, position: number, 忽略名称: string): 名称引用[] {
        ...
        function 回调(n: Node): { 名称: string, 节点: Node } {
            ...
            function 回调子节点(cn: Node): { 名称: string, 节点: Node } {
                if (isDeclarationName(cn) || isStringLiteral(cn)) {
                    if (isIdentifier(cn) && 正则.test(idText(cn)) && !忽略的名称映射.has(idText(cn))) {
                        return { 名称: idText(cn), 节点: cn };
                    }
                    else
                        if (isStringLiteral(cn) && 正则.test(cn.text) && !忽略的名称映射.has(cn.text)) {
        ...
        const 源文件组 = 程序集.getSourceFiles();
        const 过滤组 = 源文件组 && 源文件组.filter(s => s.isDeclarationFile);

        const 结果 = FindAllReferences.getReferenceEntriesForNode(找到名称.节点.getStart(), 找到名称.节点, 程序集, 过滤组, 取消);
    ...
    function 转换条目(entry: FindAllReferences.Entry): { kind: ScriptElementKind, fileName: string, textSpan: TextSpan, isInString: boolean, parent: Position } {
        if (entry.type === "span") {
            ...
            return { kind: ScriptElementKind.unknown, fileName, textSpan, isInString: false, parent: undefined };
        }
        const { node } = entry;
        const fileName = entry.node.getSourceFile().fileName;

        const 父节点 = getContainerNode(node);
        const kind = 查找种类(node);
        const 容器位置 = 转换位置为行数及字符偏移(父节点.getSourceFile(), 父节点.getStart());
        const 位置 = getTextSpan(node);
